...
export function ScheduleTable() {
  ...
  async function handleCreateSlots() {
    ...
    try {
      ...
      const ids = await createSlots(selectedDate, startHour, endHour, intervalMinutes, 1, startMinute, endMinute)
      toast.success(`Se crearon ${ids.length} horarios`)
      setShowConfig(false)
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between gap-2">
        <div className="flex items-center gap-1 sm:gap-2">
          <Button variant="ghost" size="icon" onClick={() => changeDate(-1)} className="hover:bg-foreground/5 h-8 w-8">
            <ChevronLeft className="h-4 w-4" />
          </Button>
          <input
            type="date"
            className="px-2 sm:px-3 py-2 rounded-lg border border-foreground/20 text-sm focus:outline-none focus:ring-2 focus:ring-primary-medium/50 w-36 sm:w-auto"
            value={selectedDate}
        ...
        >
          ...
          <span className="hidden sm:inline">Crear </span>horarios
        </Button>
      </div>

      {/* Date label */}
      <p className="text-sm text-foreground/50 capitalize -mt-2">{formatDate(selectedDate)}</p>

      {/* Config panel */}
      {showConfig && (
        <div className="p-4 sm:p-5 bg-primary-medium/5 border border-primary-medium/10 rounded-xl space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            ...
            <div>
              ...
              <input type="number" min="1" max="60" value={intervalMinutes} onChange={e => setIntervalMinutes(parseInt(e.target.value))}
                className="w-full px-3 py-2 border border-foreground/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-medium/50" />
            </div>
          </div>
          <p className="text-sm text-foreground/50">
            Slots de {startTime} a {endTime} cada {intervalMinutes} min
          </p>
          <div className="flex gap-2">
            <Button onClick={handleCreateSlots} disabled={creating} className="bg-primary-medium hover:bg-primary-dark text-white">
      ...
      {slots.length > 0 && (
        <div className="grid grid-cols-3 gap-2 sm:gap-3">
          <div className="px-3 py-2 sm:px-4 sm:py-3 bg-background border border-foreground/10 rounded-xl text-center">
            ...
            <span className="block text-xl sm:text-2xl font-bold text-foreground">{slots.length}</span>
          </div>
          <div className="px-3 py-2 sm:px-4 sm:py-3 bg-background border border-foreground/10 rounded-xl text-center">
            <span className="block text-xs text-foreground/50 uppercase tracking-wide font-semibold">Disponibles</span>
            <span className="block text-xl sm:text-2xl font-bold text-green-600">{availableCount}</span>
          </div>
          <div className="px-3 py-2 sm:px-4 sm:py-3 bg-background border border-foreground/10 rounded-xl text-center">
            <span className="block text-xs text-foreground/50 uppercase tracking-wide font-semibold">Ocupados</span>
            <span className={`block text-xl sm:text-2xl font-bold ${occupiedCount > 0 ? "text-amber-500" : "text-foreground/30"}`}>
              {occupiedCount}
            </span>
          </div>
        </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-foreground/50">
          ...
          <button onClick={() => setShowConfig(true)} className="text-sm text-primary-medium hover:underline mt-1">
            Crear horarios
          </button>
        </div>
      ) : (
                ...
                <tr key={slot.id} className="border-b border-foreground/5 hover:bg-foreground/2">
                  ...
                  <td className="py-3 px-4">
                    <div className="flex items-center gap-2">
                      <span className="text-foreground/70 text-xs">{slot.bookedPizzas}/{slot.maxPizzas}</span>
                      <div className="w-14 h-1.5 bg-foreground/10 rounded-full overflow-hidden hidden sm:block">
                        <div
